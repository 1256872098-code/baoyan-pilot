...
function getCompetitionTitle(item) {
  return item.nationalName || `竞赛项目 ${item.order}`;
}
    ...
    <div className="min-h-[calc(100svh-4rem)] bg-slate-50 py-9 sm:py-12">
      <div className="container-page">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-base leading-7 text-slate-600 sm:text-lg">
              按 A+、A、B、C 类浏览 2026 年国家级竞赛清单，点击项目即可查看详情。
            </p>
          </div>
              ...
              {filteredCompetitions.map((competition) => {
                ...
                return (
                  <button
                    key={competition.id}
                    type="button"
                    className="group flex min-h-[190px] flex-col rounded-xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-brand-300 hover:shadow-soft focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2"
        ...
        <div className="mt-10 rounded-xl border border-blue-100 bg-blue-50/70 px-4 py-3 text-sm leading-6 text-brand-700">
          清单依据《{competitionData.sourceTitle}》整理，仅展示原表“国家级竞赛名称”列中的项目；分类与原表中的 A+、A、B、C 类保持一致。竞赛介绍将陆续补充。
        </div>
      </div>
