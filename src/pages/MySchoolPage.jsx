...
export default function MySchoolPage() {
  ...
  const policyCardStyle = policyCardHeight ? { height: `${policyCardHeight}px` } : undefined;
  const bonusCardStyle = bonusCardHeight ? { height: `${bonusCardHeight}px` } : undefined;

  useEffect(() => {
    if (!binding || editing || typeof window === "undefined") {
      setRecommendationCardHeight(null);
      setPolicyCardHeight(null);
      setBonusCardHeight(null);
      return undefined;
    }

    const mediaQuery = window.matchMedia("(min-width: 1024px)");
    const updateAlignedCardHeights = () => {
      ...
      const overviewHeight = overviewCardRef.current?.getBoundingClientRect().height || null;
      const rankingHeight = rankingRuleCardRef.current?.getBoundingClientRect().height || null;
      const trendHeight = trendCardRef.current?.getBoundingClientRect().height || null;
      setRecommendationCardHeight(overviewHeight ? Math.round(overviewHeight) : null);
      setPolicyCardHeight(rankingHeight ? Math.round(rankingHeight) : null);
      setBonusCardHeight(trendHeight ? Math.round(trendHeight) : null);
    };

    updateAlignedCardHeights();

    const observer =
      typeof ResizeObserver !== "undefined" && (overviewCardRef.current || rankingRuleCardRef.current || trendCardRef.current)
        ? new ResizeObserver(updateAlignedCardHeights)
        : null;
    if (observer && overviewCardRef.current) observer.observe(overviewCardRef.current);
    if (observer && rankingRuleCardRef.current) observer.observe(rankingRuleCardRef.current);
    if (observer && trendCardRef.current) observer.observe(trendCardRef.current);

    window.addEventListener("resize", updateAlignedCardHeights);
    mediaQuery.addEventListener?.("change", updateAlignedCardHeights);
  ...
  const renderBindingForm = () => (
    <Card className="p-6">
      <CardHeader
        eyebrow="院校绑定"
        title="绑定本科院校"
        description="绑定后可以集中查看本校推免政策、推免数据和学校评价。"
      />
    ...
    <div className="bg-slate-50 py-10">
      <div className="container-page">
        <CardHeader
          eyebrow="个人院校"
          title="我的院校"
          description="绑定你的本科院校和学院后，可以集中查看本校推免政策、推免率、名额、排名规则和历年趋势。"
        />
          ...
          {binding && !editing && (
            <>
              <div className="grid gap-6 lg:grid-cols-2 lg:items-start">
                ...
                <div className="my-school-recommendation-equalizer" style={recommendationCardStyle}>
                  <Card className="my-school-recommendation-card h-full p-6">
              ...
              <div className="grid gap-6 lg:grid-cols-2 lg:items-start">
                <div className="my-school-policy-equalizer" style={policyCardStyle}>
                  <Card className="my-school-policy-card h-full p-6">
                ...
                <div className="my-school-bonus-equalizer" style={bonusCardStyle}>
                  <Card className="my-school-bonus-card h-full p-6">
                    <h2 className="text-xl font-bold text-slate-950">加分项目</h2>
                    {bonusRules.length ? (
                      <div className="my-school-bonus-scroll mt-4 space-y-4">
                        {bonusRules.map((rule) => (
